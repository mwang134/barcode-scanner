...
1. Manager takes a picture of the product box containing:
   * Product name (visible on box)
   * Barcode numbers (visible on box)
2. System extracts product name and barcode(s) from the image using OCR and barcode detection
3. Manager reviews/edits extracted information and assigns SKU (or system auto-generates)
4. Product is saved with name, SKU, and barcode mapping
5. Receiver scans barcode → system identifies the product instantly.

**Requirements**

* Capture photo of product box (camera/image upload)
* Extract product name from image (OCR)
* Extract barcode numbers from image (barcode detection)
* Edit/verify extracted product name
* Assign or auto-generate SKU / model number (unique)
* Edit/verify extracted barcode mappings
...
* Login / authentication (skip for MVP)
...
1. Product Setup (photo-based: capture product box image)
